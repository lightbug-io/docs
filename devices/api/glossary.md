...
| bBytes             | A byte array that represents a length and that amount of data in bytes.<br>The length prefix is a `uint8` (1 byte) for data type IDs 0–127, or a little-endian `uint16` (2 bytes) for data type IDs 128–255.<br>A bByte entry for a sub-128 field might be `3 9 9 9`, where `3` is the length, and `9 9 9` is the data.<br> A bByte entry for a greater than 128 field might be `3 0 9 9 9`, where `3` is the length, and `9 9 9` is the data.<br> See [structure](/devices/api/protocol/structure#bbytes). |
